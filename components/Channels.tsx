...
import { StyleSheet, Image, ScrollView, Pressable, Text, useWindowDimensions, View } from 'react-native';
        ...
        <ScrollView style={styles.channels} horizontal showsHorizontalScrollIndicator={false}>
            {loading ? (
                ...
                Array.from({ length: 5 }).map((_, index) => (
                    <View key={index} style={[styles.channel, { width: imageWidth, height: imageHeight + 30 }]}>
                        <View style={[styles.skeletonImage, { width: imageWidth, height: imageHeight }]} />
                    </View>
                ))
            ) : (
...
const styles = StyleSheet.create({
    ...
    skeletonImage: {
        ...
        borderRadius: 10,
        opacity: 0.6, // Slightly transparent for a skeleton effect
    },
});
